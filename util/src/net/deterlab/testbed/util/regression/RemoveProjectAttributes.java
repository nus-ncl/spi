...
/** 
 * This test calls RemoveProject in the Projects service.  It confirms that
 * RemoveProject checks its parameters then Uses it to remove the attributes added
 * by ProjectAttributes.
 * @author DETER team
 * @version 1.0
 */
public class RemoveProjectAttributes extends RegressionTest {
    /**
     * Create a new regression test
     * @param l the interactive log
     */
    public RemoveProjectAttributes() { 
	super();
    }
     ...
     */
    public String getName() { return "RemoveProjectAttributes"; }
	...
	p.print("<!-- \n" +
	    "This test calls RemoveProject in the Projects service.\n"+
	    "It confirms that RemoveProject checks its parameters then\n"+
	...
	removeProjectAttribute(pStub, null, "no name",
		p, false, DeterFault.request);
	removeProjectAttribute(pStub, "testattr2", "doesn't exist",
		p, false, DeterFault.request);

	regressionLogin(uStub, "testnotadmin", "test", p);
	removeProjectAttribute(pStub, "test-required", "bad permssions",
	...
	// Remove existing attributes (the ones created in ProjectAttributes)
	regressionLogin(uStub, "testadmin", "test", p);
	for (String attr: new String[] {
	    "test-required", "test-readonly", "test-format", "test-open" } ) {

	    removeProjectAttribute(pStub, attr, "good", p, true,
		    DeterFault.none);
	}
	regressionLogout(uStub, p);
	p.flush();
